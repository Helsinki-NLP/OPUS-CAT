...
1. Download the [OPUS-CAT MT Engine](https://github.com/Helsinki-NLP/OPUS-CAT/releases/download/engine_v1.1.0.4/OpusCatMtEngine_v1.1.0.4.zip). Create an empty folder on your computer (for instance in your Documents folder), and extract the contents of the zip file there. OPUS-CAT MT Engine does not require separate installation, it will run directly from the folder in which it has been extracted. **IMPORTANT**: OPUS-CAT MT Engine generates the machine translation, and all OPUS-CAT plugins require that the OPUS-CAT MT Engine is running on the same computer when the plugins are used. 
