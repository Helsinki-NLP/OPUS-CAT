...
OPUS-CAT MT models can be fine-tuned by using a custom batch task contained in the OPUS-CAT Trados plugin. (models can also fine-tuned directly in the OPUS-CAT MT Engine, but the Trados plugin fine-tuning allows for more targeted selection of fine-tuning sentences).
...
**Note on projects with multiple target languages**: **OPUS-CAT Finetune** batch task can be performed for only target language at a time. If you have multiple target languages, deselect the files for all but one target language before proceeding. If there are too many files to deselect, open the **Files** view of Trados Studio:
